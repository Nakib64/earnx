...
import { Check, X, Clock, Send, ShieldCheck, DollarSign, AlertCircle } from 'lucide-react';
...
export default function AdminApprovalsPage() {
  ...
  const fetchQueue = async () => {
    try {
      const data = await apiFetch('/admin/requests/pending', { isAdmin: true });
  ...
  const handleApproveActivation = async (id: string) => {
    ...
    setMsg(null);
    try {
      await apiFetch(`/admin/requests/activation/${id}/approve`, { method: 'POST', isAdmin: true });
  ...
  const handleApproveWithdrawal = async (id: string) => {
    ...
    setMsg(null);
    try {
      await apiFetch(`/admin/requests/withdrawal/${id}/approve`, { method: 'POST', isAdmin: true });
      setMsg({ type: 'success', text: 'Approved withdrawal! Deducted funds from user ledger balance.' });
      await fetchQueue();
    } catch (err: any) {
      setMsg({ type: 'error', text: err.message || 'Withdrawal approval failed' });
    } finally {
      setProcessingId(null);
    }
  };
  ...
  const handleRejectWithdrawal = async (id: string) => {
    ...
    setMsg(null);
    try {
      await apiFetch(`/admin/requests/withdrawal/${id}/reject`, { method: 'POST', isAdmin: true });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-card rounded-2xl p-5 space-y-4 border-2 border-emerald-100">

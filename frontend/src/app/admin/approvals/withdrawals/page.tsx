...
import { Check, X, DollarSign, ShieldAlert, Wallet, Search, ChevronLeft, ChevronRight, Filter } from 'lucide-react';
...
export default function AdminWithdrawalApprovalsPage() {
  ...
  const pendingWithdrawalCount = useMemo(
    () => withdrawals.filter(req => req.status === 'PENDING').length,
    [withdrawals]
  );
  ...
  const handleApproveWithdrawal = async (id: string) => {
    setProcessingId(id);
    setMsg(null);
    const res = await apiFetch<any>(`/admin/requests/withdrawal/${id}/approve`, {
      method: 'POST',
      isAdmin: true,
    });

    if (res.success) {
      setMsg({ type: 'success', text: 'Approved withdrawal! Wallet balance deducted and commission paid to sponsor.' });
  ...
  const handleRejectWithdrawal = async (id: string) => {
    ...
    const res = await apiFetch(`/admin/requests/withdrawal/${id}/reject`, {
      method: 'POST',
      isAdmin: true,
      body: JSON.stringify({ reason: rejectReason.trim() || 'Admin rejected withdrawal' }),
    });

    if (res.success) {
      setMsg({ type: 'success', text: 'Withdrawal request rejected.' });
      toast.info('Withdrawal request rejected');
      setRejectingId(null);
      setRejectReason('');
      await fetchQueue();
    } else {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto p-4 sm:p-6">
      ...
      {/* WITHDRAWAL REQUESTS QUEUE & HISTORY */}
      <div className="glass-card rounded-2xl p-5 space-y-4 bg-white border border-slate-200 shadow-xs">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-3">
          <h3 className="font-bold text-slate-900 text-sm flex items-center space-x-2">
            ...
            <span className="ml-1 px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 text-[10px] font-extrabold">
              {filtered.length}
            </span>
          </h3>
          <span className="text-xs text-slate-400 font-medium hidden sm:block">
            All historical withdrawal request records
          </span>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3">
          ...
          <div className="relative flex-1 w-full">
            ...
            <input
              ...
              placeholder="Search by user name, phone, referral code, amount, or sponsor..."
              className="w-full pl-9 pr-9 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-transparent transition-all"
            />
            {search && (
              <button
                onClick={() => setSearch('')}
          ...
          <div className="flex items-center space-x-2 w-full sm:w-auto shrink-0">
            ...
            <select
              ...
              className="bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-xs font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-400 transition-all w-full sm:w-auto"
            >
              <option value="ALL">All Statuses</option>
              <option value="PENDING">Pending Approval ({pendingWithdrawalCount})</option>
        ...
        {/* Content */}
        {loading ? (
          <div className="text-xs text-slate-400 py-10 text-center animate-pulse">Loading withdrawal records...</div>
        ) : filtered.length === 0 ? (
          <div className="text-xs text-slate-400 py-10 text-center border border-dashed border-slate-200 rounded-xl bg-slate-50">
            {search || statusFilter !== 'ALL'
              ...
              : 'No withdrawal request records found.'}
          </div>
        ) : (
          <>
            {/* Request rows */}
            <div className="space-y-3">
              {paginated.map((req) => (
                <div
                  key={req.id}
                  className="bg-slate-50 border border-slate-200 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-all hover:border-emerald-300 hover:shadow-sm"
                >
                  <div className="space-y-1">
                    ...
                    <div className="text-xs text-slate-600 font-mono space-x-3">
                      <span>Amount: <strong className="text-emerald-700 font-extrabold">৳{Number(req.amount).toLocaleString('en-US', { minimumFractionDigits: 2 })}</strong></span>
                      <span>•</span>
                      <span>Phone: <strong>{req.user?.phone}</strong></span>
                      <span>•</span>
                      <span>Balance: <strong>৳{Number(req.user?.wallet_balance || 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}</strong></span>
                    </div>
                    ...
                    {req.status === 'PENDING' ? (
                      <>
                        <button
                          ...
                          className="bg-gradient-to-r from-emerald-500 to-emerald-600 text-white hover:from-emerald-600 hover:to-emerald-700 px-4 py-2 rounded-xl text-xs font-bold flex items-center space-x-1.5 shadow-sm disabled:opacity-60 transition-all"
                        >
                          <Check className="w-4 h-4" />
                          <span>{processingId === req.id ? 'Processing...' : 'Approve & Credit Sponsor'}</span>
                        </button>
                        <button
      ...
      {rejectingId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xs">
          <div className="bg-white border border-slate-200 rounded-2xl p-6 max-w-md w-full shadow-2xl space-y-4">
            ...
            <p className="text-xs text-slate-500">Provide an optional reason for rejecting this withdrawal request.</p>
            <textarea
              value={rejectReason}
            ...
            <div className="flex justify-end space-x-2">
              ...
              <button
                onClick={() => handleRejectApproval(rejectingId)}
  ...
  function handleRejectApproval(id: string) {

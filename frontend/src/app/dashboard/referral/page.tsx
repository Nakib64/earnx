...
import { Users, ChevronRight, UserCheck, Shield, CheckCircle, Clock, AlertCircle, Star, Award } from 'lucide-react';
...
export default function ReferralPage() {
  ...
  useEffect(() => {
    const fetchTree = async () => {
      try {
        const res = await apiFetch('/users/tree?depth=5');
        setTreeData(res);
      } catch (err: any) {
      ...
      {levels.length === 0 ? (
        ...
        <div className="space-y-6">
          {levels.map((level) => {
            ...
            return (
              <div key={level} className="glass-card rounded-2xl p-5 space-y-4">
